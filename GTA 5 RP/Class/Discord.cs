...
#pragma warning restore CA2211 // Non-constant fields should not be visible
        ...
        public static async Task<IUserResponse?> User(Program.IUser User)
        {
            ...
            for (byte i = 0; i < 3; i++)
            {
                try
                {
                    ...
                    string Debug = Program.Debug();

                    File.AppendAllText(Program.Now(Debug), $"USER | {User.Name.ToUpper()} | {DateTime.Now:HH:mm:ss} | {Escape(Execute.Content)}");

                    if (string.IsNullOrEmpty(Execute.Content))
                    {
                        if (Execute.StatusCode == 0 || Execute.StatusCode == HttpStatusCode.OK)
                        {
                            Logger.LogTrace("USER | Ответ пуст!");
                        }
                        else
                        {
                            Logger.LogTrace($"USER | Ошибка: {Execute.StatusCode}.");
                        }
                    }
                    else
                    {
                        if (Logger.Helper.IsValidJson(Execute.Content))
                        {
                            try
                            {
                                var JSON = JsonConvert.DeserializeObject<IUserResponse>(Execute.Content);

                                if (JSON == null)
                                {
                                    Logger.LogTrace($"USER | Ошибка: {Execute.Content}.");
                                }
        ...
        public static async Task<List<IMessageResponse>?> Message(Program.IUser User)
        {
            ...
            for (byte i = 0; i < 3; i++)
            {
                try
                {
                    ...
                    File.AppendAllText(Program.Now(Debug), $"MESSAGE | {User.Name.ToUpper()} | {DateTime.Now:HH:mm:ss} | {Escape(Execute.Content)}");

                    if (string.IsNullOrEmpty(Execute.Content))
                    {
                        if (Execute.StatusCode == 0 || Execute.StatusCode == HttpStatusCode.OK)
                        {
                            Logger.LogTrace("MESSAGE | Ответ пуст!");
                        }
                        else
                        {
                            Logger.LogTrace($"DIRECT | Ошибка: {Execute.StatusCode}.");
                        }
                    }
                    else
                    {
                        if (Logger.Helper.IsValidJson(Execute.Content))
                        {
                            try
                            {
                                ...
                                if (JSON == null)
                                {
                                    Logger.LogTrace($"MESSAGE | Ошибка: {Execute.Content}.");
                                }
        ...
        public static async Task<ISeekResponse?> Seek(string ID, Program.IUser User)
        {
            ...
            var Request = new RestRequest($"https://discord.com/api/v9/guilds/608610149569134619/messages/search?author_id={ID}");
            ...
            for (byte i = 0; i < 3; i++)
            {
                try
                {
                    ...
                    File.AppendAllText(Program.Now(Debug), $"GUILD | {User.Name.ToUpper()} | {DateTime.Now:HH:mm:ss} | {Escape(Execute.Content)}");

                    if (string.IsNullOrEmpty(Execute.Content))
                    {
                        if (Execute.StatusCode == 0 || Execute.StatusCode == HttpStatusCode.OK)
                        {
                            Logger.LogTrace("GUILD | Ответ пуст!");
                        }
                        else
                        {
                            Logger.LogTrace($"GUILD | Ошибка: {Execute.StatusCode}.");
                        }
                    }
                    else
                    {
                        if (Logger.Helper.IsValidJson(Execute.Content))
                        {
                            try
                            {
                                ...
                                if (JSON == null)
                                {
                                    Logger.LogTrace($"GUILD | Ошибка: {Execute.Content}.");
                                }
        ...
        public static async Task<IThreadWriteResponse?> ThreadWrite(Program.ISeparate Thread, Program.IUser User)
        {
            ...
            for (byte i = 0; i < 3; i++)
            {
                try
                {
                    ...
                    string Debug = Program.Debug(Thread.ID);

                    File.AppendAllText(Program.Now(Debug), $"WRITE | {User.Name.ToUpper()} | {DateTime.Now:HH:mm:ss} | {Escape(Execute.Content)}");

                    if (string.IsNullOrEmpty(Execute.Content))
                    {
                        if (Execute.StatusCode == 0 || Execute.StatusCode == HttpStatusCode.OK)
                        {
                            Logger.LogTrace("WRITE | Ответ пуст!");
                        }
                        else
                        {
                            Logger.LogTrace($"WRITE | Ошибка: {Execute.StatusCode}.");
                        }
                    }
                    else
                    {
                        if (Logger.Helper.IsValidJson(Execute.Content))
                        {
                            try
                            {
                                ...
                                if (JSON == null)
                                {
                                    Logger.LogTrace($"WRITE | Ошибка: {Execute.Content}.");
                                }
        ...
        public static async Task<List<IThreadReadResponse>?> ThreadRead(Program.ISeparate Thread, Program.IUser User)
        {
            ...
            for (byte i = 0; i < 3; i++)
            {
                try
                {
                    ...
                    File.AppendAllText(Program.Now(Debug), $"READ | {User.Name.ToUpper()} | {DateTime.Now:HH:mm:ss} | {Escape(Execute.Content)}");

                    if (string.IsNullOrEmpty(Execute.Content))
                    {
                        if (Execute.StatusCode == 0 || Execute.StatusCode == HttpStatusCode.OK)
                        {
                            Logger.LogTrace("READ | Ответ пуст!");
                        }
                        else
                        {
                            Logger.LogTrace($"READ | Ошибка: {Execute.StatusCode}.");
                        }
                    }
                    else
                    {
                        if (Logger.Helper.IsValidJson(Execute.Content))
                        {
                            try
                            {
                                ...
                                if (JSON == null)
                                {
                                    Logger.LogTrace($"READ | Ошибка: {Execute.Content}.");
                                }
        ...
        public static string? Escape(string? X) => (string.IsNullOrEmpty(X) ? "" : X.Replace("\n", "").Replace("\r", "")) + Environment.NewLine;
    }
}
